...
from car.utl.constants import PLLP_R1_GRID_WITH_BORDER, PLLP_R1_GRID
...
m.setParam("LogFile", "res1.txt")

M = len(PLLP_R1_GRID) + 1
N = len(PLLP_R1_GRID) + 1
...
for i in range(M + 1):
    for j in range(N + 1):
        m.addConstr(1 >= PLLP_R1_GRID_WITH_BORDER[i][j] + X[i, j] + Y[i, j])
...
summary, timelines, rootlp = glt.get_dataframe(["res1.txt"], timelines=True)
